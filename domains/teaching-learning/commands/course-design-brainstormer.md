...
```mermaid
graph TD
    A[Course Design Input] --> B(Phase 1: Analysis)
    B -->|Approved Analysis & Diagnostic| C(Phase 2: Design)
    C -->|Approved COs & CO-PO Map| D(Phase 3: Development)
    D -->|Approved Syllabus & Portfolio Spec| E(Phase 4: Implementation)
    E -->|Approved Classroom Delivery Plan| F(Phase 5: Evaluation)
    F -->|Approved Assessment & Rubrics| G[Final Improved Course Package]
