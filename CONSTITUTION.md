...
> For **what** the platform does, see `specification/architecture.md`. For **how** it is built — domains, agents, skills, commands, memory architecture — see `specification/design.md`. This file does not repeat either; it states the conventions and guardrails that govern how those two documents, and the folders they describe, may be changed.
...
| Primary build artifact | The two vision documents (`specification/architecture.md`, `specification/design.md`) and the plain-markdown `domains/` they describe |
| Distribution | A single read-only shared OneDrive folder, opened directly by **Claude Code**, **GitHub Copilot in VS Code**, or **Google Antigravity** — no installation, no manifest, no registry |
...
- The agent **may read** `CONSTITUTION.md`, `AGENTS.md`, `specification/architecture.md`, and `specification/design.md` if it needs them for context (for example, to correctly name a file per convention) — reading governance files is not restricted.
...
The full current folder layout is defined and kept up to date in `specification/design.md` §Repository Structure. This Constitution does not duplicate that tree. It states only the **rules about the layout**, which are stable even as the layout itself evolves:
...
- Anything described in `specification/design.md` as living in a user's personal `srujana-memory/` must never be created inside the shared `SrujanaSangama/` folder, and vice versa.
...
| Layer | Convention |
|---|---|
| Domain folder names | `kebab-case`, matching exactly the names used in `specification/design.md` (e.g. `teaching-learning`, `kaizen-excellence`) |
...
| Domain agent routers | Optional `domains/<domain>/AGENTS.md` files may act as compact loading indexes for that domain. They may name commands, rules, skills, and memory locations, but must not duplicate substantive command, rule, skill, `specification/architecture.md`, or `specification/design.md` content. |
| Host instruction shims | `.github/copilot-instructions.md` and `GEMINI.md` may exist as host-specific pointers to the same routing protocol. They must stay thin and must not become manifests, registries, or parallel documentation. |
...
| All proposed changes to `specification/architecture.md` or `specification/design.md` | Drafted first in `specification/` (see §6) before the source documents are edited |

**Rule:** Never silently duplicate content that already exists in `specification/architecture.md` or `specification/design.md` into a domain's `README.md` or any other file. Reference it instead (e.g. "see specification/architecture.md, Domain 3").
...
- State explicitly, near the top, what the agent drafts and what the human must decide or approve before acting on the output — matching the human-AI collaboration principle in `specification/architecture.md`
...
`specification/` is where changes to the platform are proposed, reviewed, and staged before `specification/architecture.md`, `specification/design.md`, or any `domains/` content is actually edited. It is only accessible in Development Mode (§2.3).

```
specification/
├── specification/architecture.md    ← What the platform does
├── specification/design.md          ← How the platform is built
...
- A proposal file must state: the problem, the proposed change to `specification/architecture.md` and/or `specification/design.md`, an explicit Scope Boundaries section (in scope / out of scope), and a Verification section with testable acceptance criteria.
- No domain content, skill, or validator is created or edited until its proposal is approved by the Product Owner.
- Approved proposals are reflected back into `specification/architecture.md` and/or `specification/design.md` directly — `specification/` is a staging area, not a permanent parallel description of the system. Once merged, the proposal file may be archived but the live description of the system remains solely in the two vision documents.
- **`IMPLEMENTATION-STATUS.md` is the one exception to "vision documents describe the whole system."** `specification/architecture.md` and `specification/design.md` describe the complete target system regardless of build progress. `IMPLEMENTATION-STATUS.md` tracks the gap between that target and what actually exists on disk today, and is what `README.md` and `AGENTS.md` defer to rather than overstating current capability.
...
| Item | Rule |
|---|---|
| Spec-first | A change to `specification/` is committed before the corresponding edit to `specification/architecture.md`, `specification/design.md`, or `domains/` |
...
| Creating a packaged-extension manifest, install script, or registry | The platform deliberately has none of these — see `specification/design.md` Overview. Reintroducing one is a Constitution violation, not an enhancement |
