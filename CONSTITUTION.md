...
| Primary build artifact | Markdown specification files (`plan/`) |
...
│   ├── plan-*.prompt.md         ← Feature spec files (approved by Product Owner before implementation)
...
| Spec files | Markdown (`.prompt.md` extension in `plan/`) | Word documents, Notion exports |
...
| Spec files | `plan-<camelCaseName>.prompt.md` | `plan-revaPhDScholar.prompt.md` |
| Task files | `tasks-<camelCaseName>.md` | `tasks-revaPhDScholar.md` |
...
| Item | Rule |
|---|---|
| Spec-first | Change the spec in `plan/` first, commit it, then let AI regenerate affected files |
