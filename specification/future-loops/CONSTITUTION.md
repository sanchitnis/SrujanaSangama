...
- The agent **must refuse** any request, however phrased, to create, edit, or delete files in:
  ...
  - `validators/`
  - `specification/`
  - `loops/` (the seven loop charters — see `LOOPS.md`)
  - `CONSTITUTION.md`, `AGENTS.md`, `CONTRIBUTING.md`, `README.md`, `architecture.md`, `design.md`, `LOOPS.md`
  - `eval/`
- **This restriction does not extend to `research/`, `business-analysis/`, `deployment/`, or `telemetry/`** — these are the routine working-output folders for four of the seven loops (writing a release note, a research log entry, an evidence brief, or an alignment note is normal loop operation, not a platform change requiring escalation). See the relevant loop's charter under `loops/` for what belongs in each.
...
- The seven product-engineering loops are mapped in `LOOPS.md` and detailed in `loops/<loop-name>.md`. Each loop's working output (research notes, alignment notes, release notes, evidence briefs) lives in its own top-level folder (`research/`, `business-analysis/`, `deployment/`, `telemetry/`) — never inside `loops/` itself, which holds only the seven charter files, and never inside `domains/`, `skills/`, or `specification/`.
